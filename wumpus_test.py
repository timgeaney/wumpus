...
def magic_arrow():
    global arrows
    global wumpus_location
    print "select up to 3 nr caves to shoot into. All caves must be linked!!"
    shot =get_caves()
    ...
    if is_connected(player_location, shot[0], shot[1], shot[2]) and check_for_intersection(shot, wumpus_location):
        ...
        print "list of shots: ", shot
        print "Is there an intersection: ", check_for_intersection(shot, wumpus_location)

        set_wumpus_lives(1)

        answer = set(shot)
        kill_shot = answer.intersection(wumpus_location)
        print "which wumpus bought if? ; ", kill_shot
        for shot in kill_shot:
            print shot
            wumpus_location.remove(shot)
            print "you have", wumpus , "left to kill!"
            print "Wumpus list: ", wumpus_location
        return wumpus_location
...
print "wumpus Location", wumpus_location
print "bat location", bat_location
